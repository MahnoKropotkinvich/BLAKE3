...
// Test whether the C compiler can compile RVV intrinsics using
fn test_rvv_compile_support() -> bool {
    use std::io::Write;

    let out_dir = env::var("OUT_DIR").unwrap();
    let test_c = format!("{}/test_rvv_probe.c", out_dir);
...
void blake3_rvv_probe(void) {
    size_t vl = __riscv_vsetvlmax_e32m1();
    vuint32m1_t a = __riscv_vmv_v_x_u32m1(0, vl);
    (void)__riscv_vadd_vv_u32m1(a, a, vl);
}
";

    let mut f = match std::fs::File::create(&test_c) {
        Ok(f) => f,
        Err(_) => return false,
    };
    if f.write_all(test_code).is_err() {
        return false;
    }
    drop(f);
    ...
    build.file(&test_c);
    build.try_compile("blake3_rvv_probe").is_ok()
}
